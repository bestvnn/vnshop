<?php
$_count = count(getData('core_comment_categories1','id,category_id,category_name','','','id','desc',''));
$perPage = 10;
$_data = getData('core_comment_categories1','id,category_id,category_name','','','id','desc',$perPage);
$comment_category = getData('core_offers','id,name','','','id','desc','');
?>
<section class="row">
  <div class="col-md-6">
    <h2 class="section-heading mb-4">Comment Category Sub 1 (<?php echo $_count; ?>)</h2>
  </div>
</section>
      ...
      <table style="margin-top:10px;" id="dtBasicExample" class="table table-hover table-bordered" cellspacing="0" width="100%">
        <thead>
          <tr>            
            ...
            <th class="text-center" data-toggle="tooltip" title="ID"><strong>#ID</strong></th>
            <th class="text-center" data-toggle="tooltip" title="Commtent Category"><strong>Comment Category</strong></th>
			      <th class="text-center" data-toggle="tooltip" title="Name"><strong>Name</strong></th>                
          </tr>
        </thead>
        <tbody id="content">    
            <?php 
              foreach ($_data as $row) {       
                ...
                echo '<a href="?route=editCommtentCategoryOne&id='.$row["id"].'"><span class="btn btn-warning btn-sm waves-effect waves-light"><i class="w-fa fas fa-edit ml-1"></i></span></a>';
                ...
                if(!empty($comment_category)){
                    echo '<td><strong class="trigger green lighten-3" style="color:#fff;">'.$comment_category["name"].'</strong></td>';
                }else{
                    echo '<td></td>';
                }
	      ...
	      <div class="row">
	        <div class="col-md-12">
	          <div class="md-form form-sm mb-0">	            
                <select id="category_id" name="category_id" class="mdb-select md-form">
					          <option value="-1">-- Chọn Comment Category --</option>  
                    <?php 
	        ...
	        <div class="col-md-12">
	          <div class="md-form form-sm mb-0">
	            <input id="category_name" type="text" class="form-control form-control-sm">
	            <label for="category_name" data-toggle="tooltip" title="Key dùng để so sánh trong api(có thể bỏ trống)">Category Name</label>
...
$(document).ready(function(){
    $("#pageSize").on('change',function(){
      ...
      $.ajax({
        ...
        type:"POST",
        data:{limit : limit},            
        url:'<?=$_url;?>/ajax.php?act=pagination-commentCategoryOne',
    ...
    $("#newCommentCategory1").on("click","button[type=submit]",function(){                
            ...
            if(category_id.val()=="-1"){
	            ...
	            $.ajax({
	                url: '<?=$_url;?>/ajax.php?act=admincp-newCommentCategory1',
	                dataType: 'json',
    ...
    $("#cancelOrder").on("click","[name=cancelOrder]",function(){    
        ...
        $.ajax({
            url: '<?=$_url;?>/ajax.php?act=cancelCommentCategoryOne',
            dataType: 'json',
