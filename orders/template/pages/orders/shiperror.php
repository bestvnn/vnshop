...
$perPage = 10;
if (isAller()) {
    $_count = $_db->query("select * from `core_orders` where `status` = '" . $_status . "' " . $sql . " and (`time` >= '" . (strtotime(str_replace("/", "-", $ts) . " GMT+7 00:00")) . "' and `time` < '" . (strtotime(str_replace("/", "-", $te) . " GMT+7 23:59")) . "') and `user_ship`!='' " . $_sql_offer . " " . $sql)->num_rows();
    $_data = $_db->query("select * from `core_orders` where `status` = '" . $_status . "' " . $sql . " and (`time` >= '" . (strtotime(str_replace("/", "-", $ts) . " GMT+7 00:00")) . "' and `time` < '" . (strtotime(str_replace("/", "-", $te) . " GMT+7 23:59")) . "') and `user_ship`!='' " . $_sql_offer . " " . $sql . " order by id DESC LIMIT $perPage")->fetch_array();
} else {
...
<section class="row">
    <div class="col-md-6">
        <h2 class="section-heading mb-4"><span class="badge <?= getBgOrder($_status); ?>">ShipError</span> Khách không nhận hàng (<?= $_count; ?>)</h2>
    </div>
    ...
    <div class="col-md-12 mx-auto white z-depth-1">
        <div class="mb-5">
            <form name="filter" method="GET">
                ...
                <input name="type" value="shipError" type="hidden">
            ...
            <div class="postList" style="overflow-x:auto;margin-bottom:15px">
                <table id="dtBasicExample" class="table table-sm  table-bordered table-hover" cellspacing="0" width="100%">
                    ...
                    <tbody id="content">
                        ...
                        <span class="btn btn-danger btn-sm waves-effect waves-light"><i class="w-fa fas fa-edit ml-1"></i></span>
                      </a>';
                                echo '</td>';
                                echo '<td class="text-center">Đơn hàng #<b>' . $arr['id'] . '</b></td>';
                                echo '<td class="text-center"><b class="trigger red lighten-3">' . $_offer_name . '</b></td>';
    ...
    $(document).ready(function() {
        $("#pageSize").on('change', function() {
            ...
            $.ajax({
                ...
                url: '<?= $_url; ?>/ajax.php?act=pagination-orderShipError&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&view=<?php echo $view; ?>',
        ...
        loadOrderBy('<?= $_url; ?>/ajax.php?act=orderby-orderShipError&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&view=<?php echo $view; ?>');
    });
</script>
<script type="text/javascript">
