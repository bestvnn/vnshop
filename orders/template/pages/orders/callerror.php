...
$_count = $_db->query("select * from `core_orders` where `status` = '" . $_status . "' and (`time` >= '" . (strtotime(str_replace("/", "-", $ts) . " GMT+7 00:00")) . "' and `time` < '" . (strtotime(str_replace("/", "-", $te) . " GMT+7 23:59")) . "') " . $_sql_offer . " " . $sql)->fetch_array();
$perPage = 10;
...
<section class="row">
    <div class="col-md-6">
        <h2 class="section-heading mb-4"><span class="badge <?= getBgOrder($_status); ?>">CallError</span> Đơn hàng hẹn gọi lại (<?php echo $_count; ?>)</h2>
    </div>
    ...
    <div class="col-md-12 mx-auto white z-depth-1">
        <div class="mb-5">
            <form name="filter" method="GET">
                ...
                <input name="type" value="callError" type="hidden">
            ...
            <div class="postList" style="overflow-x:auto;margin-bottom:15px;">
                <table id="load_data_table" class="table table-sm  table-bordered table-hover" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" data-toggle="tooltip" title="Số điện thoại liên hệ" aria-sort="ascending">Order Phone</th>
                            <th class="text-center" data-toggle="tooltip" title="Thời gian gọi" aria-sort="ascending">Call Time</th>
                            <th class="text-left" data-toggle="tooltip" title="Ghi chú">Note</th>
                            <?php if (isLeader() || isAller()) { ?>
                                <th class="text-center" data-toggle="tooltip" title="Người gọi" aria-sort="ascending">Caller</th>
                            <?php } ?>
                        </tr>
                    </thead>
                    <tbody id="content">
                        ...
                        foreach ($_data as $arr) {
                            ...
                            echo '<td class="text-center">' . get_time($arr['call_time']) . '</td>';
    ...
    $(document).ready(function() {
        $("#pageSize").on('change', function() {
            ...
            $.ajax({
                ...
                url: '<?= $_url; ?>/ajax.php?act=pagination-orderCallError&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&view=<?php echo $view; ?>',
        ...
        loadOrderBy('<?= $_url; ?>/ajax.php?act=orderby-orderCallError&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&view=<?php echo $view; ?>');
    });
</script>
<script type="text/javascript">
