...
if(isAller()){
  ...
  $data_number = $_db->query("select * from `core_orders` where `status` = '".$_status."' and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`!='' ".$_sql_offer." ".$sql."order by `id` desc ")->fetch_array();



  $_data = $_db->query("select * from `core_orders` where `status` = '".$_status."' and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`!='' ".$_sql_offer." ".$sql."order by `id` desc limit $perPage")->fetch_array();
  ...
  $data_number = $_db->query("select * from `core_orders` where `status` = '".$_status."' and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."')  ".$_sql_offer." ".$sql."order by `id` desc")->fetch_array();
...
<section class="row">
  ...
  <div class="col-md-6">
    ...
    <h2 class="section-heading mb-4"><span class="badge <?=getBgOrder($_status);?>">Ship Failed</span> Phát chưa thành công (<?=$_count;?>)</h2>
          ...
          <?php
            ...
            if($_data){
              ...
              foreach ($_data as $arr) {
                ...
                echo '<td style="padding-left:30px;padding-right:30px;" class="text-center"><b>'.date('d-m-Y H:i',$arr['update_time']).'</b></td>';
...
<div class="modal fade" id="showExcel" tabindex="-1" role="dialog"
...
<div class="modal fade" id="makeShiperror" tabindex="-1" role="dialog"
...
<div class="modal fade" id="makeApprove" tabindex="-1" role="dialog"
  ...
  $(document).ready(function(){
      ...
      $("#showExcel").on('click','[type=submit]', function(){
            ...
            $.ajax({
                ...
                data: {id: $idImport.join(","),action: $makeImport},
      ...
      $("#pageSize").on('change',function(){
        ...
        $.ajax({
          ...
          url:'<?=$_url;?>/ajax.php?act=pagination-orderShipFail&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&view=<?php echo $view; ?>', 
...
<script type="text/javascript">
  ...
  $("#makeApprove").on('click','[type=submit]', function(){
        ...
        $.ajax({
            ...
            data: {id: $idOrder.join(","),action: "approved"},
  ...
  $("#makeShiperror").on('click','[type=submit]', function(){
        ...
        $.ajax({
            ...
            data: {id: $idOrder.join(","),action: "shiperror"},
