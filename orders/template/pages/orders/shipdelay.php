...
$perPage = 10;
if (isAller()) {
    $_count = $_db->query("select * from `core_orders` where `status` = '" . $_status . "' " . $sql . " and (`time` >= '" . (strtotime(str_replace("/", "-", $ts) . " GMT+7 00:00")) . "' and `time` < '" . (strtotime(str_replace("/", "-", $te) . " GMT+7 23:59")) . "') " . ($showall == false && !$user ? " and `user_ship`!='' " : "") . " " . $_sql_offer . " " . $sql)->num_rows();
    $_data = $_db->query("select * from `core_orders` where `status` = '" . $_status . "' " . $sql . " and (`time` >= '" . (strtotime(str_replace("/", "-", $ts) . " GMT+7 00:00")) . "' and `time` < '" . (strtotime(str_replace("/", "-", $te) . " GMT+7 23:59")) . "') " . ($showall == false && !$user ? " and `user_ship`!='' " : "") . " " . $_sql_offer . " " . $sql . " order by id DESC LIMIT $perPage")->fetch_array();
} else {
...
<section class="row">
    <div class="col-md-6">
        <h2 class="section-heading mb-4"><span class="badge <?= getBgOrder($_status); ?>">ShipDelay</span> Hẹn ngày giao hàng (<?= $_count; ?>)</h2>
    </div>
    ...
    <div class="col-md-12 mx-auto white z-depth-1">
        <div class="mb-5">
            <form name="filter" method="GET">
                ...
                <input name="type" value="shipDelay" type="hidden">
                ...
                <table id="dtBasicExample" class="table table-sm  table-bordered table-hover" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center sort-heading table_asc" id="number-asc" data-toggle="tooltip" title="Số lượng mua">Number&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
    ...
    $(document).ready(function() {
        $("#pageSize").on('change', function() {
            ...
            $.ajax({
                ...
                url: '<?= $_url; ?>/ajax.php?act=pagination-orderShipDelay&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&view=<?php echo $view; ?>',
        ...
        loadOrderBy('<?= $_url; ?>/ajax.php?act=orderby-orderShipDelay&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&view=<?php echo $view; ?>');
    });
</script>
<script type="text/javascript">
...
<?php if (isLeader() && isShipper()) { ?>
    <script type="text/javascript">
        ...
        $("body").on('click', '[role=btn-cancelAll]', function() {
            ...
            $(".loader-overlay").show();
            $.ajax({
                url: '<?= $_url; ?>/ajax.php?act=cancelOrderDelay',
                dataType: 'json',
                data: {
                    id: $idOrder.join(",")
                },
