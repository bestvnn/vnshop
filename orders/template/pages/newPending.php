...
$offer = isset($_GET['offer']) ? trim($_GET['offer']) : (isset($_COOKIE['newPending_offer']) ? $_COOKIE['newPending_offer'] : 'all');
...
$area = isset($_GET['area']) ? $_GET['area'] : (isset($_COOKIE['newPending_area']) ? $_COOKIE['newPending_area'] : 'all');
...
if(isset($_GET['offer']) && $_GET['offer'])
  ...
  setcookie("newPending_offer",$offer,time()+3600*24*365);

if(isset($_GET['area']) && in_array(trim($_GET['area']),['bac','trung','nam']))

  setcookie("newPending_area",$area,time()+3600*24*365);



if(isset($_GET['offer']) && !$_GET['offer'])

  setcookie("newPending_offer","");

if(isset($_GET['area']) && !$_GET['area'] || $_GET['area'] == "all")

  setcookie("newPending_area","");
...
$_count = $_db->query("select `id` from `core_orders` where `status` = 'pending' ".$sql." and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`='' ".$_sql_offer)->num_rows();

$perPage = 10;

$_data = $_db->query("select * from `core_orders` where `status` = 'pending' ".$sql." and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`='' ".$_sql_offer." order by id DESC LIMIT $perPage")->fetch_array();
...
<section class="row">
  ...
  <div class="col-md-6">
    ...
    <h2 class="section-heading mb-4"><span class="badge <?=getBgOrder('pending');?>">Pending</span> Đơn hàng đang chờ giao (<?=$_count;?>)</h2>
...
<section class="row mb-5 pb-3">
    ...
    <div class="col-md-12 mx-auto white z-depth-1">
      ...
      <form name="filter" method="GET">
        ...
        <input name="route" value="newPending" type="hidden">
    ...
    <div class="row">      
      ...
      <div class="col-md-2 text-right" style="margin-bottom:15px;">   

      <button role="btn-getAll" class="btn btn-info" disabled="">Get All Selected<i class="fas fa-dolly ml-1"></i></button>
    ...
    <div class="postList" style="overflow-x:auto;margin-bottom:15px;">  
      ...
      <table id="load_data_table" class="table table-sm table-hover table-bordered" cellspacing="0" width="100%">
        ...
        <thead>
          ...
          <tr>
            ...
            <th class="text-center sort-heading table_asc" id="number-desc" data-toggle="tooltip" title="Số lượng mua">Number&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
            ...
            ?>
            <th class="text-center sort-heading table_asc" data-toggle="tooltip" id="order_name-asc" title="Tên người muốn mua">Order Name&nbsp;&nbsp;&nbsp;</th>            
            ...
            <th class="text-left" data-toggle="tooltip" title="Địa chỉ mua hàng">Province</th>   
          ...
          <?php
            ...
            if($_data){
              ...
              foreach ($_data as $arr) {
                ...
                echo '<button role="btn-getOrder" class="btn '.getBgOrder('pending').' btn-sm btn-rounded waves-effect waves-light"><i class="w-fa fas fa-dolly ml-1"></i> Get Order</button>
                </td>';
                ...
                echo '<td class="text-left">'._ucwords($arr['note']).'</td>';                
                ...
                echo '<td>'._ucwords($arr['order_province']).'</td>';
                if($isShipper){
  ...
  $(document).ready(function(){
      ...
      $("body").on('click','[role=btn-getAll]', function(){
            ...
            $.ajax({
                ...
                url: '<?=$_url;?>/ajax.php?act=selectOrderPending',
      ...
      $("#pageSize").on('change',function(){
          ...
          $.ajax({
            ...
            url:'<?=$_url;?>/ajax.php?act=pagination-newPending&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&area=<?php echo $area; ?>', 
        ...
        $(document).on('click', '.load-more', function(){    
          ...
          loadOrderBy('<?=$_url;?>/ajax.php?act=orderby-newPending&ts=<?php echo $ts; ?>&te=<?php echo $te; ?>&offer=<?php echo $offer; ?>&area=<?php echo $area; ?>');                
