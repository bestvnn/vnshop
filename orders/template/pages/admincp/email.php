...
<h2 class="section-heading mb-4">Email Settings</h2>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title">Add Email Settings</h4>
	      ...
	      <div class="row">
	        <div class="col-md-12">
	          <div class="md-form form-sm mb-0">
	            ...
	            <div class="invalid-feedback">Name không được để trống trống.</div>
	          </div>
	        </div>	        
...
<div class="modal fade" id="editCommentCategory" tabindex="-1" role="dialog"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <!--Header-->
      <div class="modal-header">
        <h4 class="modal-title">Edit Email Marketing</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Cancel">
          <span aria-hidden="true">×</span>
        </button>
      </div>
...
<section class="row mb-5 pb-3">
    <div class="col-md-12 mx-auto white z-depth-1" style="overflow-x: hidden;">
    	<div class="text-right pb-2">
  			<button role="btn-newCommentCategory" class="btn btn-primary" data-toggle="modal" data-target="#newCommentCategory">Add New<i class="fas fa-plus-square ml-1"></i></button>
  		</div>
      <table id="dtBasicExample" class="table table-bordered table-sm table-hover" cellspacing="0" width="100%">
        ...
        <tbody>
          <?php
            ...
            $_data = $_db->query("select * from `core_email_settings` order by `id` DESC ")->fetch_array();
            ...
            if($_data){
              ...
              foreach ($_data as $arr) {
                ...
                <tr data-id="<?php echo $arr['id'];?>" data-name="<?php echo _e($arr['name']);?>" data-status="<?php echo $arr['status'];?>" data-key="<?php echo _e($arr['key_api']);?>">
                    ...
                    <td class="text-center"><b style="color:#fff;" class="trigger <?php if($arr['status'] == '1'){ echo 'blue';}else{ echo 'red';} ?>"><?php echo $arr["name"]; ?></b></td>
                    <td class="text-center"><b class="number"><?php echo $arr["key_api"]; ?></b></td>
                    <td class="text-center">
                        <button role="btn-editCommentCategory" type="button" class="btn btn-info"  data-toggle="modal" data-target="#editCommentCategory"> Edit <i class="fas fa-pen-square ml-1"></i></button>
                        <button role="btn-deleteCommentCategory" type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteCommentCategory"> Delete <i class="fas fa-times ml-1"></i></button>
                    </td>
                <tr>
                <?php
              }			  
	...
	$(document).ready(function(){
        ...
        $("#newCommentCategory").on("click","button[type=submit]",function(){            
	        ...
	        if(!name.val().trim()){
	            ...
	            $.ajax({
	                url: '<?=$_url;?>/ajax.php?act=admincp-newEmailSetting',
	                dataType: 'json',
        ...
        $("#editCommentCategory").on("click","button[type=submit]",function(){    
	        ...
	        if(!name.val().trim()){
	            ...
	            $.ajax({
	                url: '<?=$_url;?>/ajax.php?act=admincp-editEmailSetting',
	                dataType: 'json',
        ...
        $("#deleteCommentCategory").on("click","button[type=submit]",function(){
            ...
            $.ajax({
                url: '<?=$_url;?>/ajax.php?act=admincp-deleteEmailMarketing',
                dataType: 'json',
                data: {id: $idOffer},
                type: 'post',
                success: function (response) {
