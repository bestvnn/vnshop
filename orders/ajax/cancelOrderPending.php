...
$sql = "";
if(!isLeader())
	$sql = " `id` in ('".implode("','", $exp_id)."') and `status`='pending' and `user_ship`='".$_user['id']."' ";
else {

	if(isAller()){
		$sql = " `id` in ('".implode("','", $exp_id)."') and `status`='pending' and `user_ship`!='' ";
	} else {
		  $memGroup = memberGroup($_user['group']);
		  $user_ship = array();
		  foreach ($memGroup as $us)
		    $user_ship[] = $us['id'];
		$sql = " `id` in ('".implode("','", $exp_id)."') and `status`='pending' and `user_ship` in ('".implode("','", $user_ship)."') ";		
	}
