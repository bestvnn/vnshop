...
$orders = $_db->query("select * from `core_orders` where `id` in ('".implode("','", $exp_id)."') and `status`='pending' and `user_ship`='' ")->fetch_array();

if(!$orders)
	exit('{"status":403,"message":"Không tìm thấy đơn hàng!"}');


if($_db->exec_query("update `core_orders` set `user_ship`='".escape_string($_user['id'])."',`ship_time`='".time()."' where `id` in ('".implode("','", $exp_id)."') and `status`='pending' and `user_ship`='' ")){
