...
if(isAller()){
  ...
  $result = $_db->query("select * from `core_orders` where `status` = '".$_status."' ".$sql." and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`!='' ".$_sql_offer." ".$sql." order by $columnName $sortOrder LIMIT $perPage")->fetch_array();
...
foreach ($result as $row) {   
    ...
    $paginationHtml.= '<td class="text-center"><b class="trigger red lighten-3">'._e($row['offer_name']).'</b></td>';
    ...
    $paginationHtml.= '<td class="text-left">'._e($row['order_phone']);
