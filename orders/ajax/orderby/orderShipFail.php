...
if(isAller())
  ...
  $result = $_db->query("select * from `core_orders` where `status` = '".$_status."' and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`!='' ".$_sql_offer." ".$sql." order by $columnName $sortOrder LIMIT $perPage")->fetch_array();


else


  $result = $_db->query("select * from `core_orders` where `status` = '".$_status."' and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') ".$_sql_offer." ".$sql." order by $columnName $sortOrder LIMIT $perPage")->fetch_array();
...
foreach ($result as $row) {   
    ...
    $paginationHtml.= '<td style="padding-left:30px;padding-right:30px;" class="text-center"><b>'.date('d-m-Y H:i',$row['update_time']).'</b></td>';
    if(!empty($row['parcel_code'])){
        $paginationHtml.= '<td class="text-center">'.$row['parcel_code'].'<a target="_blank" href="http://www.vnpost.vn/en-us/dinh-vi/buu-pham?key='.$row['parcel_code'].'"><i class="fas fa-external-link-alt ml-1"></i></a></td>';
    }else{
        $paginationHtml.= '<td class="text-center"></td>';
    }       
    $paginationHtml.='<td class="text-left">';
    $paginationHtml.='<p style="margin-bottom:0;"> '.nl2br(_ucwords($row['order_info'])).'</p>';    
    $paginationHtml.='</td>';
    ...
    $paginationHtml.= '<td class="text-left"> '._ucwords($arr['order_province']).'</td>';
