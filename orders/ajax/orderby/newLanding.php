...
$sql .= (!isAdmin() && isPublisher()) ? " and `ukey`= '".$_user['ukey']."'" : '';
...
$result = $_db->query("select * from `core_landing_stats` where `date` in ('".implode("','", $list_time)."') ".$sql." order by $columnName $sortOrder LIMIT $perPage")->fetch_array();
$paginationHtml = '';
$total_viewPage = 0;
$total_order = 0;
$total_count_apo = 0;
