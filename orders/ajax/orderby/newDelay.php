...
$result = $_db->query("select * from `core_orders` where `status` = 'shipdelay' ".$sql." and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`='' ".$_sql_offer." order by $columnName $sortOrder LIMIT $perPage")->fetch_array();
$paginationHtml = '';
