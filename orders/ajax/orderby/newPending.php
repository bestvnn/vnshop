...
$result = $_db->query("select * from `core_orders` where `status` = 'pending' ".$sql." and (`time` >= '".(strtotime(str_replace("/","-", $ts)." GMT+7 00:00"))."' and `time` < '".(strtotime(str_replace("/","-", $te)." GMT+7 23:59"))."') and `user_ship`='' ".$_sql_offer." order by $columnName $sortOrder LIMIT $perPage")->fetch_array();
...
foreach ($result as $arr) {
    ...
    if(isAller())
        ...
        $paginationHtml.= '<a href="?route=editOrder&id='.$arr['id'].'">

              <span class="btn btn-warning btn-sm waves-effect waves-light"><i class="w-fa fas fa-edit ml-1"></i></span>

            </a>';

    $paginationHtml.= '<button role="btn-getOrder" class="btn '.getBgOrder('pending').' btn-sm btn-rounded waves-effect waves-light"><i class="w-fa fas fa-dolly ml-1"></i> Get Order</button>
    ...
    $paginationHtml.= '<td class="text-center">x<b>'._e($arr['number']).'</b></td>';

    $paginationHtml.= '<td class="text-center"><b>'.addDotNumber($arr['price_sell']).'</b>k</td>';

    $paginationHtml.= '<td class="text-center">'.date('Y/m/d H:i',$arr['time']).'</td>';
    if(isAdmin()){
      $paginationHtml.= '<td class="text-center">'.$arr['landing'].'</td>';
    }
    $paginationHtml.= '<td class="text-center">'._ucwords($arr['order_name']).'</td>';
    ...
    $paginationHtml.= '<td class="text-left">'._ucwords($arr['note']).'</td>';
    ...
    $paginationHtml.= '<td>'._ucwords($arr['order_province']).'</td>';
    if($isShipper){
